...
def is_summary_expired(summary_updated_at: datetime | None) -> bool:
    ...
    if summary_updated_at.tzinfo is None:
        summary_updated_at = summary_updated_at.replace(tzinfo=timezone.utc)

    return now - summary_updated_at > timedelta(days=SUMMARY_TTL_DAYS)
